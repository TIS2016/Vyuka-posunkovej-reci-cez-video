...
class Player(Tk.Frame):
    ...
    def __init__(self, parent, title=None):
        """constructor creates main panel"""
        ...
        # The video panel, holds main video its controls and timeslider
        # video
        self.player = None
        ...
        # The record panel, holds video from web-cam, and its controls
        # web-cam video
        ...
        # canvas which hold list of recorded videos
    ...
    def OnOpen(self):
        ...
        # Create a file dialog opened in the home directory
    ...
    def export(self):
        ...
        try:
            ...
            pass
        os.system("mpeg\\ffmpeg -i " + self.fullpath + " -s 640x480 raw.mp4")
        for i in self.recvidslist:
            ...
            try:
                ...
                pass
            os.system("mpeg\\ffmpeg -i video" + j + ".avi video" + j + ".mp4")
        ...
        for i in range(len(results)):
            if i == 0: res = results[0]
            else:
                res = concatenate_videoclips([res, results[i]])
        zoz = os.listdir(os.path.dirname(self.fullpath))
        mam = False
        cislo = 0
        nazov = os.path.basename(self.fullpath).split(".")[0] + "-opravene.mp4"
        while mam is not True:
            if cislo is not 0: nazov = os.path.basename(self.fullpath).split(".")[0] + "-opravene(" + str(cislo) + ").mp4"
            if nazov in zoz:
                cislo += 1
                continue
            mam = True
        ...
        print(nazov)
        vys = os.path.dirname(self.fullpath) + "/" + nazov
        res.write_videofile(vys)
